...
| `/snake-draft` | **Snake draft** — practice snake draft for *other* (non-Ottoneu) redraft leagues: pick the number of teams, your draft slot, the starting lineup and the number of rounds, then draft against AI opponents off the Draft Sharks consensus board re-priced as value over replacement for that format. Same manager-valuation-noise slider as the mock draft. Gated on projections access. See [docs/references/snake-draft.md](references/snake-draft.md) |
