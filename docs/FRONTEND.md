...
| `/players` | Tabbed: **Directory** (searchable player list) + **Efficiency** (PPG/PPS-vs-salary scatter with a `?season=` picker; the min-games slider caps at the games played so a fifteen-stop range does not blank the chart mid-season). The individual player card (`/players/[id]`) prints an `Earned $` column across the season-stats table plus a latest-season header tile, and the shared `PlayerHoverCard` prints the latest season's earned value beside the projection — both behind `hasProjectionsAccess` (`buildHoverDataMap` defaults the field off so a future public caller cannot leak it). See [references/player-valuation.md](references/player-valuation.md) |
...
| `/weekly` | **Per-game weekly projections board** — Sleeper-sourced, in-season projections for one NFL week (defaults to the current week per `web/lib/nfl-week.ts`), rendered from `weekly_projections`. Deliberately distinct from the season-long `/projections` route: `projected_points` is one game's third-party market-aware forecast, not the season-long PPG the model produces. Also surfaces per-player last-week actuals when the row carries them (refreshed daily 7am ET, plus a Sunday 16:00 UTC pre-kickoff pass — #705). Gated on projections access. See [docs/references/weekly-projections.md](references/weekly-projections.md) |
| `/projection-accuracy` | Model backtest accuracy explorer |
| `/value` | Tabbed: **Earned** (realized-value board + per-team scoreboard — leads in-season because it is the only panel here that makes no claim about football that has not happened yet) · **VORP** (bar chart + table) · **Surplus** (rankings, bargains, overpaid, team summaries) · **Adjustments** (per-user manual value overrides). Once the season is finished the retrospective Earned tab moves to the end and VORP is again the entry point. Carries a `?season=` picker (past seasons available) and a `<StatWindowNote>` on every production-reading panel so a two-game sample cannot masquerade as a full year. See [references/player-valuation.md](references/player-valuation.md) |
...
| `/podcast/recap` | **Weekly recap** (podcaster only) — episode prep for the week that just finished: the six results against each side's projected total (an *upset* is the lower-projected side winning), top performers, the biggest surprises against the kickoff-frozen projections, the biggest surprises against that week's consolidated ranking (with each host's own placement), and what every roster left on its bench in hindsight. All derived at read time in `web/lib/weekly-recap.ts` — nothing stored. Defaults to the last **finished** week. See [docs/references/podcast-tools.md](references/podcast-tools.md#weekly-recap) |
...
| `/power-rankings` | **Community power rankings** (public) — every locked-in ballot, hosts *and* listeners, consolidated by the reveal's own mean-rank maths (`web/lib/community-rankings.ts`). A week appears once published: automatically Thursday 09:00 ET, or early/held by a host from `/podcast`. Hosts can open an unpublished week as a labelled preview. Rows carry no per-voter fields — only the overall, host and listener averages, range and first-place votes |
| `/pickem` | **Weekly pick'em** (public board; picking needs a sign-in) — pick the winner of every matchup in the current week. Each tap saves on its own (`PUT /api/pickem/pick`); tapping the picked team clears it. Players choose a board name first (`PUT /api/pickem/name`, prefilled with their bound team). Picks lock Thursday 20:00 ET (noon on Thanksgiving). Before the lock the page shows only your sheet and a player count; after it, each game's split (who picked which side, green/struck-through once final) and the week's standings — points for correct picks of final games, ties push, shared ranks. `?week=N` opens past weeks. Logic in `web/lib/pickem.ts`; see [matchups-and-standings.md](references/matchups-and-standings.md#weekly-pickem) |
| `/power-rankings/vote` | **Listener ballot** — signed-in accounts that are *not* podcasters rank the league for the current week (drag or up/down, autosaved draft, lock in to count). Voting closes when the week is published. Hosts are pointed at their podcast ballot, which already counts. `PUT /api/power-rankings/ballot` |
