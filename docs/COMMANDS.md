...
just scrape-lineups [--week N] [--dry-run]            # Scrape each game's box score (both lineups, slots, live points) into matchup_lineups (drives /scoreboard/[gameId] + live projections)
...
| `pull-matchups.yml` | Daily 11:00 UTC + every 30 min through the Sunday afternoon/evening and the Thursday-, Sunday- and Monday-night windows + `workflow_dispatch` | `scripts/scrape_matchups.py` — the league schedule and live/final scores into `league_matchups` — then `scripts/scrape_lineups.py` — the live week's box scores into `matchup_lineups`. Deliberately **not** gated on the season: the schedule page is public and cheap year-round, and the daily off-season run is what catches next season's schedule the day it posts |
| `pull-weekly-projections.yml` | Daily 11:00 UTC (~7am ET) + Sundays 16:00 UTC (pre-kickoff) + `workflow_dispatch` | `scripts/weekly_projections/ingest.py` — per-game projections from Sleeper. The daily run also pulls actuals for the current and previous week; the Sunday run is projections-only (games in progress). Projections are **gated on `is_weekly_ingest_window()`** — open daily from 1 September through Week 18, so the Week 1 board refreshes through the run-up to kickoff and the job still no-ops all offseason. Actuals are gated separately on `is_nfl_week_live()` (nothing to pull before kickoff). A manual run skips both gates |
...
| `pull-player-stats.yml` | Tuesdays 14:00 UTC (Sep–Feb) + `workflow_dispatch` (optional seasons input) | `scripts/enqueue.py player-stats` + `scripts/worker.py` — keeps `player_stats` current for the season being played. The target season is resolved from `league_calendar` at run time via `scripts.season.stats_season()`, never hardcoded. Tuesday because the NFL week flips Tuesday 00:00 ET and nflverse republishes `stats_player_reg_{year}.parquet` after Monday night. Before this existed nothing scheduled ever wrote `player_stats`, so the table stopped at the last hand-run backfill and every stats-joined page went blank at the season rollover |
...
| `backfill-player-stats.yml` | `workflow_dispatch` only (seasons input) | Historical Ottoneu `player_stats` backfill. For the *current* season use `pull-player-stats.yml` instead — this one is for filling in past seasons |
