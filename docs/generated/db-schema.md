...
| `weekly_projections` | In-season **per-game** projections for one NFL week, ingested from Sleeper by `scripts/weekly_projections/ingest.py` and re-scored under Ottoneu rules. Columns: `projected_points`, `projected_stats` (jsonb), `opponent`, `game_date` (the NFL game's date — once past, the projection is **frozen** as the pre-kickoff forecast; migration 043), `actual_points`, `actual_stats`, `source`. Actuals share the row so a played week shows projection vs result. Retention: current season only. Distinct from `player_projections` (our season-long PPG model) and must never feed it (FK -> `players`) | `(player_id, season, week, source)` |
| `league_matchups` | The league's head-to-head game log — one row per Ottoneu game, keyed by Ottoneu's own `game_id`. Columns: `season`, `week`, `home_team_id`/`home_team_name`/`home_score`, the away equivalents, `status` (`scheduled`/`in_progress`/`final`), `game_type` (`regular`/`playoff`/`championship`/`third_place`/`consolation` — only `regular` counts toward the standings), `starts_on`/`ends_on` (the fantasy week's window) and `status_label` (Ottoneu's own words, verbatim). Scraped from the public `/schedule` page plus the `/csv/schedule` export by `scripts/scrape_matchups.py`. **Standings are derived from these rows at read time** (`web/lib/standings.ts`), not stored — see [references/matchups-and-standings.md](../references/matchups-and-standings.md). Migration 037. RLS enabled with an anon SELECT policy so the public `/scoreboard` page and the homepage can read it. | `(league_id, game_id)` |
| `matchup_lineups` | Who each team actually started in each game — one row per (game, player), bench included, scraped from Ottoneu's public box score (`/football/{league}/game/{game_id}`) by `scripts/scrape_lineups.py`. Columns: `season`, `week`, `game_id`, `side` (`home`/`away`), `team_id`/`team_name`, `ottoneu_id` + `player_id` (FK -> `players`, nullable), `player_name`, `nfl_team`, `position`, `slot` (`QB`/`RB`/`WR`/`TE`/`K`/`FLEX`/`SFLX`/`BN`), `slot_number`, `is_starter`, `points` (NULL until his game starts), `game_state` (`scheduled`/`in_progress`/`final`/`bye`), `game_info` (Ottoneu's words, verbatim), `injury_status`, `stat_line`. Ottoneu's own Proj column is deliberately **not** stored (it becomes the actual after a game). The live matchup projection is derived at read time (`web/lib/live-matchup.ts`) — see [references/matchups-and-standings.md](../references/matchups-and-standings.md#lineups-and-the-live-matchup-projection). Migration 044. RLS with an anon SELECT policy. | `(league_id, game_id, ottoneu_id)` |
...
| `power_ranking_ballots` | One account's power-ranking ballot for one NFL week (FK -> `users`). `submitted_at` NULL = still a private draft, and excluded from consolidation. `voter_kind` (migration 045) is `host` or `listener`, stamped from `is_podcaster` at save time: the podcast reveal counts `host` ballots only (`fetchBallots` defaults to that scope), the public community ranking counts both. Migrations 041, 045. Server-only (service key); RLS enabled, no anon policy — ballots are private by construction. See [podcast-tools.md](../references/podcast-tools.md). | `(league_id, season, week, user_id)` |
| `power_ranking_publications` | Per-week override of when the public community power ranking goes live. No row = on the schedule (Thursday 09:00 ET of that week); `published_at` set = a host published it early; `held` = held back past Thursday until published by hand (the two are mutually exclusive by CHECK). Publication also closes listener voting for the week. Migration 045. Server-only (service key); RLS enabled, no anon policy. See [podcast-tools.md](../references/podcast-tools.md#community-power-rankings). | `(league_id, season, week)` |
| `power_ranking_entries` | One team's slot on one ballot (FK -> `power_ranking_ballots`): `rank` (1 = best) plus two independent free-text fields: `note`, the one-liner read out on the show, and `prep_note` (migration 042), the host's private working notes on that team — never revealed, never consolidated, and returned only by `fetchPrepNotes`, which is scoped to one `user_id`. Ranks are unique per ballot (deferrable), so a ballot is a total order rather than a scattering of opinions. Migrations 041, 042. Server-only (service key); RLS enabled, no anon policy. | `(ballot_id, team_name)`, `(ballot_id, rank)` |
| `pickem_picks` | One account's weekly pick'em pick for one league matchup (FK -> `users`): `season`, `week`, `game_id` and `picked_team_id` (Ottoneu's own ids, as in `league_matchups` — deliberately **no FK** to it, so a re-drawn schedule never cascade-deletes picks). A week locks Thursday 20:00 ET (noon on Thanksgiving), computed in `web/lib/pickem.ts`; results are derived from `league_matchups` at read time, not stored. Nobody else's picks are shown before the lock. Migration 046. Server-only (service key); RLS enabled, no anon policy. See [matchups-and-standings.md](../references/matchups-and-standings.md#weekly-pickem). | `(league_id, game_id, user_id)` |
| `pickem_players` | The name a pick'em player appears under on the public board (FK -> `users`): `display_name`, 2–30 chars, unique per league case-insensitively (index on `lower(display_name)`). A league team's name is reserved for its own manager (checked in the API). Migration 046. Server-only (service key); RLS enabled, no anon policy. | `(league_id, user_id)` |
...
- `pickem_players.user_id` -> `users.id`
