...
| `team_vegas_lines` | Per-team-season Vegas implied total + Pythagorean win total, aggregated from nflverse `games.csv`. `implied_total` is nullable (migration 025) so preseason win totals can be seeded before the NFL schedule releases per-game lines | `(team, season)` |
