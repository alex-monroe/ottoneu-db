...
| `player_contracts` | Per-player NFL contract history scraped from OverTheCap per-position contract-history pages by `scripts/scrape_otc_contracts.py` (the nflverse contracts mirror is stale, ends 2022). One row per (player, `year_signed`); columns `years`, `value`, `apy`, `guaranteed`, `apy_cap_pct` (OTC "APY as % of cap at signing" — cap-inflation normalized). A `year_signed`-dated, leakage-free offseason role-securing signal (FA opens mid-March) for the `new_contract_value_raw` / `contract_signed_raw` features (spike #651 source #3). Python-only read (service key); RLS enabled, no anon policy. | `(player_id, year_signed)` |
