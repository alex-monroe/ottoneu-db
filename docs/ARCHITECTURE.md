...
**Important dependency:** All internal models share a `weighted_ppg`-family base feature (`weighted_ppg` for v1–v11, `weighted_ppg_no_qb_trajectory` for v12 onward). Changing `WeightedPPGFeature.RECENCY_WEIGHTS` (or the `NoQBTrajectory` variant's weights) requires regenerating projections for **every** internal model that uses it via `cli.py run`, not just re-running backtests. The `--run-backtest` flag on `accuracy_report.py` only re-backtests existing projections — it does not regenerate them.
