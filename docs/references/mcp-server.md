...
- `get_arbitration_analysis` and `analyzeArbitration()` (`web/lib/arbitration.ts`) both **parameterize** the excluded team since #712 — the site version takes `myTeam` from `getViewerTeam()`, the MCP variant (`analyzeArbTargets` in `web/lib/mcp/arb.ts`) takes `excludeTeam` explicitly. The two stay separate because `analyzeArbTargets` returns the smaller MCP-shaped result, but a parity test still asserts they agree for the same `excludeTeam`. **When the request carries a per-user OAuth token, `exclude_team` defaults to the caller's own `users.team_name`** via `callerTeam(extra)` (`web/lib/mcp/tools.ts:576`), so an agent no longer inherits the operator's viewpoint. The shared `MCP_API_KEY` belongs to nobody and defaults to no exclusion (league-wide view); pass an empty string for `exclude_team` to force the league-wide view even from an OAuth caller.
