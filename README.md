...
- `league_prices`: League-specific current salaries (FK to `players`, unique per player/league)
- `transactions`: Structured event log of all Ottoneu transactions (adds, cuts, trades, auctions) parsed from player card pages, with salary and date data for historical tracking
