...
- **Roster CSV Reconciliation:** See [docs/references/roster-csv-reconciliation.md](docs/references/roster-csv-reconciliation.md) for the primary roster-state sync (`just reconcile-roster`) — ingests Ottoneu's `/csv/rosters` export into `league_prices` (+ optionally inferred `transactions`). This is the roster ingest since #691 removed the Playwright scrape; `scripts/scrape_player_cards.py` (plain HTTP per DB player id) provides the real transaction history alongside it. Runs via `.github/workflows/pull-roster-csv.yml` (daily/on-demand). Note: the CSV endpoint works from GitHub-hosted runners as long as an **honest User-Agent** is sent (spoofing a browser UA — not the datacenter IP — is what trips Cloudflare); `OTTONEU_COOKIE` is only an optional fallback. The CSV has no FAs/transaction history
...
- **Route access & viewer team:** See [docs/ARCHITECTURE.md § Authentication & Authorization](docs/ARCHITECTURE.md#authentication--authorization) and [§ Teams as objects](docs/ARCHITECTURE.md#teams-as-objects). Route gating is one Edge-safe module (`web/lib/access.ts` — `PROJECTIONS_ROUTES`, `ADMIN_ROUTES`, `PODCASTER_ROUTES`, `PUBLIC_API_ROUTES`, `accessRedirect()`); a signed-in user without projections access goes to `/access`, never `/login` (registration grants `has_projections_access: false`, so the reverse produced an infinite redirect loop — pinned by `web/__tests__/lib/access.test.ts`). Add a new gated route to `PROJECTIONS_ROUTES` (or `PODCASTER_ROUTES` for the podcast tools — but never `/podcast` itself, which is the page that explains the gate) and call the matching `require*Access()` from the page rather than hand-rolling a check. **"My team" is per-user, not a constant** — resolve it with `getViewerTeam()` (`web/lib/viewer-team.ts`); `config.MY_TEAM` is the operator's default only, applied to an unbound admin account. Every user-visible team name should render through `TeamName` / `fantasyTeamCol()` so it links to `/teams/[name]` — bare team strings are a regression
...
│   ├── [roster-csv-reconciliation.md](docs/references/roster-csv-reconciliation.md)  # /csv/rosters → league_prices sync (just reconcile-roster); the transaction state machine + INFERRED_MARKERS guard, checked by `just check-transactions`
