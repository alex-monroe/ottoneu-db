...
- **Route access & viewer team:** See [docs/ARCHITECTURE.md § Authentication & Authorization](docs/ARCHITECTURE.md#authentication--authorization) and [§ Teams as objects](docs/ARCHITECTURE.md#teams-as-objects). Route gating is one Edge-safe module (`web/lib/access.ts` — `PROJECTIONS_ROUTES`, `ADMIN_ROUTES`, `PUBLIC_API_ROUTES`, `accessRedirect()`); a signed-in user without projections access goes to `/access`, never `/login` (registration grants `has_projections_access: false`, so the reverse produced an infinite redirect loop — pinned by `web/__tests__/lib/access.test.ts`). Add a new gated route to `PROJECTIONS_ROUTES` and call `requireProjectionsAccess()` from the page rather than hand-rolling a check. **"My team" is per-user, not a constant** — resolve it with `getViewerTeam()` (`web/lib/viewer-team.ts`); `config.MY_TEAM` is the operator's default only, applied to an unbound admin account. Every user-visible team name should render through `TeamName` / `fantasyTeamCol()` so it links to `/teams/[name]` — bare team strings are a regression
