...
- **MCP Server:** See [docs/references/mcp-server.md](docs/references/mcp-server.md) for the remote MCP endpoint (`/api/mcp/mcp`) that exposes league data (rosters, projections, values, arbitration) to MCP clients/AI agents. Dual auth: shared key via `MCP_API_KEY`, or per-user OAuth 2.1 (the site is its own authorization server — `web/lib/oauth/`, gated on `has_projections_access`) for clients like Gemini Spark that require it. Tool registry in `web/lib/mcp/tools.ts`; the transaction feed's chronological ordering + `feed_quality` bulk-load flag + FA-salary nulling all live in `web/lib/mcp/transactions.ts` (#699 "Consumer-facing gotchas"); client provisioning via `just oauth-client`
- **Roster CSV Reconciliation:** See [docs/references/roster-csv-reconciliation.md](docs/references/roster-csv-reconciliation.md) for the plain-HTTP roster-state sync (`just reconcile-roster`) — ingests Ottoneu's `/csv/rosters` export into `league_prices` (+ inferred transactions, deduped 14 days back against the `scrape_player_cards.py` transaction log so a reconciliation never files a same-day duplicate of a scraped move — #699, with an after-the-fact `scripts/transaction_dedupe.py` pass that runs at the end of every card-scrape `--apply` to converge duplicates the pre-write filter cannot catch when the CSV job runs before the card scrape — #702). This is the primary roster ingest path since #691 removed the Playwright scrape. Runs daily via `.github/workflows/pull-roster-csv.yml`, alongside `scrape-player-cards.yml` (which fetches per-player transaction history over HTTP). Note: the CSV endpoint works from GitHub-hosted runners as long as an **honest User-Agent** is sent (spoofing a browser UA — not the datacenter IP — is what trips Cloudflare); `OTTONEU_COOKIE` is only an optional fallback. The CSV has no FAs / transaction history (transactions come from the player-card scrape; FA average salaries currently have no automated refresh path)
- **Weekly Projections:** See [docs/references/weekly-projections.md](docs/references/weekly-projections.md) for the in-season **per-game** projections (`just weekly-projections`) — ingested from Sleeper's public read-only API (free for non-commercial use; honest User-Agent, no key, no scraping), re-scored under Ottoneu rules into `weekly_projections`. Deliberately distinct from the seasonal model: `projected_points` (one game, third-party, market-aware) vs `projected_ppg` (season-long, ours, market-free). **Weekly projections must never feed `scripts/feature_projections/`** — enforced by `TestNoWeeklyProjectionsInModel`. NFL week is resolved by `scripts/nfl_week.py` / `web/lib/nfl-week.ts` (twins), flipping every **Tuesday 00:00 ET**. Refresh cadence (#705): daily 7am ET pulls projections + in-progress week actuals so Monday morning shows Sunday's results, plus a Sunday 16:00 UTC projections-only pass after inactives drop
...
- **Autonomous operation:** See [docs/references/autonomous-operation.md](docs/references/autonomous-operation.md) for the permission-friction strategy — allowlist design, prompt-rate metrics (`just permission-report`), and the `.devcontainer/` for safely running `claude --dangerously-skip-permissions` (also documents the egress firewall's rotating-IP handling for `api.github.com` / npm registry, IPv6 REJECT policy, and the `.mcp.json.example` MCP-server template)
- **Season cycle:** See [docs/exec-plans/season-cycle.md](docs/exec-plans/season-cycle.md) — the site rolls between Ottoneu seasons from the `league_calendar` table; the current season is resolved at runtime via `scripts/season.py` and `web/lib/season.ts`, not from static config. Five phases (`pre_arb` / `pre_keeper` / `pre_draft` / `post_draft` / `in_season`); the `post_draft` split is conditional on `auction_date` being set *and* strictly past, so leagues that never publish a draft date stay entirely on `pre_draft` between keeper deadline and kickoff (#700). `SEASON_OVERRIDE` / `PHASE_OVERRIDE` env vars are the manual escape hatch
...
│   └── [season-cycle.md](docs/exec-plans/season-cycle.md)                # Cross-season data & UI scheme (date-driven season-cycle resolver, 5 phases incl. post_draft after #700)
├── generated/
...
│   ├── [roster-csv-reconciliation.md](docs/references/roster-csv-reconciliation.md)  # /csv/rosters → league_prices sync (just reconcile-roster) — primary roster ingest since #691 removed the Playwright scrape (14-day dedupe vs scraped card moves, #699)
...
│   ├── [weekly-projections.md](docs/references/weekly-projections.md) # In-season per-game projections from Sleeper; NFL-week resolver; never feeds the model (daily + Sunday 16:00 UTC pre-kickoff refresh — #705)
│   ├── [snake-draft.md](docs/references/snake-draft.md)                # /snake-draft: public, non-Ottoneu snake draft off The Athletic's checked-in VORP board (+ per-format baseline re-derivation, shared valuation-noise slider — #701)
│   ├── [mcp-server.md](docs/references/mcp-server.md)                  # Remote MCP endpoint (/api/mcp/mcp): league-data tools for AI agents; dual auth (shared bearer key + per-user OAuth 2.1); consumer-facing gotchas (transaction feed, FA salaries, season context) live in web/lib/mcp/transactions.ts
...
1. **Run the held-out re-rank** against the active production model (currently `v44_eb_pooling_perpos` as of 2026-07 — always confirm from `projection_models.is_active`, do not assume) and the naïve baselines, on the rolling-origin protocol (#594). Learned models retrain out-of-sample inside the sandbox; additive/external models need their held-out projections generated first (`just project <name> 2023,2024,2025`). The cache (#597) makes re-runs fast.
   ```
   just holdout-eval --protocol rolling --eval-seasons 2023,2024,2025 --min-train-season 2021 \
     --models <name>,v44_eb_pooling_perpos,naive_prior_season_ppg,position_mean_baseline
   ```
2. **Test significance vs the active model** (player-clustered paired bootstrap). This is the gate — a point-estimate MAE delta is **not** a result:
   ```
   just significance <name> v44_eb_pooling_perpos --protocol rolling --eval-seasons 2023,2024,2025 --min-train-season 2021
   ```
...
Supabase's Python client defaults to a **1000-row limit** on `.execute()` calls. Any query that may return more than 1000 rows must use paginated `.range(offset, offset + page_size - 1)` fetching in a loop. This has caused silent bugs in `promote.py`, `analysis_utils.fetch_multi_season_stats` (a 3-season history fetch is ~2k rows — truncation silently dropped player-season rows, corrupting weighted-PPG bases and projections), `feature_projections/backtest.py` (a single recent season of `player_stats` now exceeds 1000 rows), and `roster_context_pack.py` (#692 — `league_prices` grew past 1000 rows once #691's CSV reconcile started writing ~876 FA rows alongside rostered players, so unpaginated reads truncated the snapshot to a partial subset of teams) — all now fixed. A single recent NFL season of `player_stats` is already >1000 rows, so even single-season fetches need pagination now. Apply the same pattern in any new bulk-fetch code.
...
**This is now mechanically enforced (#620).** `TestSupabasePagination` in `scripts/tests/test_architecture.py` and the "Supabase Pagination" suite in `web/__tests__/lib/architecture.test.ts` statically scan for non-paginated reads against the large tables (`player_stats`, `nfl_stats`, `depth_charts`, `model_projections` — the `LARGE_TABLES` list in each test, kept in sync across the two languages) and fail `just check-arch` if one is found. **Known gap:** `league_prices` also exceeds 1000 rows now (post-#691) and #692 flagged adding it to `LARGE_TABLES` as a recommended follow-up — until that lands, paginate `league_prices` reads manually (via `fetch_all_rows` / `fetchAllRows`); the scanner won't catch a bare `.execute()` against it. Read through the paginated helpers instead of a bare `.table(...).execute()` / `.from(...).select(...)`:
