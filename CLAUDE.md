...
- **Snake Draft:** See [docs/references/snake-draft.md](docs/references/snake-draft.md) for `/snake-draft` — a **non-Ottoneu** practice snake draft for other redraft leagues (configurable teams / draft slot / lineup / rounds, same manager-valuation-noise slider). The Draft Sharks superflex board is re-priced as value over replacement for whatever lineup you configure, which is what makes it format-aware
...
│   ├── [snake-draft.md](docs/references/snake-draft.md)                # /snake-draft: non-Ottoneu practice snake draft for other redraft leagues
