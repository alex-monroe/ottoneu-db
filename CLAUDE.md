...
│   ├── [roster-csv-reconciliation.md](docs/references/roster-csv-reconciliation.md)  # /csv/rosters → league_prices sync (just reconcile-roster) — primary roster ingest since #691 removed the Playwright scrape
│   ├── [mcp-server.md](docs/references/mcp-server.md)                  # Remote MCP endpoint (/api/mcp/mcp): league-data tools for AI agents; dual auth (shared bearer key + per-user OAuth 2.1)
